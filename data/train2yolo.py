...
if __name__ == '__main__':
    ...
    original_path = current_path+sys.argv[1]
    print(original_path)
    ...
    for i in range(len(aa.imgs_path)):
        ...
        if img is not None:
            ...
            with open(save_txt_path, "w") as f:
                ...
                annotations = np.zeros((0, 38))
                if len(labels) == 0:
                    continue
                for idx, label in enumerate(labels):
                    annotation = np.zeros((1, 38))
                    # bbox
                    ...
                    ######################################################luson add
                    annotation[0, 14] = label[19] / width  # l0_x
                    annotation[0, 15] = label[20] / height  # l0_y
                    annotation[0, 16] = label[22] / width  # l1_x
                    annotation[0, 17] = label[23] / height  # l1_y
                    annotation[0, 18] = label[25] / width  # l2_x
                    annotation[0, 19] = label[26] / height  # l2_y
                    annotation[0, 20] = label[28] / width  # l3_x
                    annotation[0, 21] = label[29] / height  # l3_y
                    annotation[0, 22] = label[31] / width  # l4_x
                    annotation[0, 23] = label[32] / height  # l4_y
                    annotation[0, 24] = label[34] / width  # l0_x
                    annotation[0, 25] = label[35] / height  # l0_y
                    
                    annotation[0, 26] = label[37] / width  # l1_x
                    annotation[0, 27] = label[38] / height  # l1_y
                    annotation[0, 28] = label[40] / width  # l2_x
                    annotation[0, 29] = label[41] / height  # l2_y
                    annotation[0, 30] = label[43] / width  # l3_x
                    annotation[0, 31] = label[44] / height  # l3_y
                    annotation[0, 32] = label[46] / width  # l4_x
                    annotation[0, 33] = label[47] / height  # l4_y
                    annotation[0, 34] = label[49] / width  # l0_x
                    annotation[0, 35] = label[50] / height  # l0_y
                    annotation[0, 36] = label[52] / width  # l1_x
                    annotation[0, 37] = label[53] / height  # l1_y
