...
def non_max_suppression_face(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, labels=()):
    ...
    nc = prediction.shape[2] - 15+24  # number of classes
    ...
    output = [torch.zeros((0, 16+24), device=prediction.device)] * prediction.shape[0]
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if labels and len(labels[xi]):
            ...
            v[range(len(l)), l[:, 0].long() + 15+24] = 1.0  # cls
            x = torch.cat((x, v), 0)
        ...
        # Compute conf
        x[:, 15+24:] *= x[:, 4:5]  # conf = obj_conf * cls_conf
        ...
        if multi_label:
            i, j = (x[:, 15+24:] > conf_thres).nonzero(as_tuple=False).T
            x = torch.cat((box[i], x[i, j + 15+24, None], x[i, 5:15+24] ,j[:, None].float()), 1)
        else:  # best class only
            conf, j = x[:, 15+24:].max(1, keepdim=True)
            x = torch.cat((box, conf, x[:, 5:15+24], j.float()), 1)[conf.view(-1) > conf_thres]
        ...
        # Batched NMS
        c = x[:, 15+24:16+24] * (0 if agnostic else max_wh)  # classes
